...
def load_and_process_instalados(_file_path=EXCEL_FILE):
    ...
    try:
        ...
        if raw_df.empty:
            st.error("Nenhum dado disponível para a aba 'Instalados'. Verifique o nome da aba no arquivo Excel.")
            return pd.DataFrame()
        ...
        if missing_columns:
            for col in missing_columns:
                col_type = SHEET_CONFIG['Instalados']['columns'][col].get('type', 'string')
                if col_type == 'date':
                    raw_df[col] = pd.NaT
...
def render_instalados(uploaded_file=None):
    ...
    if df.empty:
        st.error("Nenhum dado disponível para a aba 'Instalados'. Verifique o arquivo Excel e o nome da aba.")
        return
    ...
    with tab2:
        st.markdown("### Dashboard de Instalados")
        ...
        # Gráfico 1: PARECER DA VISITA TÉCNICA
        st.markdown("#### Distribuição de Cidades por Parecer da Visita Técnica")
        valid_parecer = SHEET_CONFIG['Instalados']['columns']['PARECER DA VISITA TÉCNICA']['values'] + ["Não Informado"]
        df_grafico['PARECER DA VISITA TÉCNICA'] = df_grafico['PARECER DA VISITA TÉCNICA'].apply(
            lambda x: x if x in SHEET_CONFIG['Instalados']['columns']['PARECER DA VISITA TÉCNICA']['values'] else "Não Informado"
        )
        ...
        # Gráfico 2: APTO PARA INSTALAÇÃO?
        st.markdown("#### Distribuição de Cidades por Aptidão para Instalação")
        df_grafico['APTO PARA INSTALAÇÃO?'] = df_grafico['APTO PARA INSTALAÇÃO?'].map({True: 'Sim', False: 'Não'})
        df_counts_apto = df_grafico['APTO PARA INSTALAÇÃO?'].value_counts().reset_index()
        df_counts_apto.columns = ['Apto', 'Contagem']
        for category in ['Sim', 'Não']:
            if category not in df_counts_apto['Apto'].values:
                df_counts_apto = pd.concat([df_counts_apto, pd.DataFrame({'Apto': [category], 'Contagem': [0]})], ignore_index=True)
        
        if not df_counts_apto.empty and df_counts_apto['Contagem'].sum() > 0:
            fig_apto = px.pie(
                df_counts_apto,
                names='Apto',
                values='Contagem',
                title='Cidades por Aptidão para Instalação',
                height=400,
                color='Apto',
                color_discrete_map={'Sim': '#00CC96', 'Não': '#EF553B'}
            )
            fig_apto.update_traces(textinfo='percent+label', textposition='inside', showlegend=True)
            fig_apto.update_layout(legend_title_text='Apto', margin=dict(t=50, b=50, l=50, r=50))
            st.plotly_chart(fig_apto, use_container_width=True)
        else:
            st.warning("Nenhum dado disponível para o gráfico de aptidão para instalação.")
        
        # Relatório de Datas (idêntico ao de ag_instalacao)
        ...
        st.download_button(
            ...
            data=excel_data,
            file_name="relatorio_datas_instalados.xlsx",
