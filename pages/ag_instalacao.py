import streamlit as st
import pandas as pd
from utils.data_utils import load_excel, parse_training_period, parse_date_columns

def render_ag_instalacao():
    st.subheader("Aguardando Instalação")
    
    try:
        df = load_excel('Ag_Instalacao')
        
        expected_columns = [
            'CIDADE', 'SIT. DA INFRA-ESTRUTURA P/VISITA TÉCNICA', 'PARECER DA VISITA TÉCNICA',
            'REALIZOU TREINAMENTO?', 'SITUAÇÃO DO NOVO TERMO DE COOPERAÇÃO',
            'DATA DO D.O.', 'APTO PARA INSTALAÇÃO?', 'DATA DA INSTALAÇÃO',
            'DATA DO INÍCIO ATEND.'
        ]
        missing_columns = [col for col in expected_columns if col not in df.columns]
        if missing_columns:
            st.warning(f"Colunas ausentes na aba 'Ag_Instalacao': {', '.join(missing_columns)}")

        if 'PERÍODO PREVISTO DE TREINAMENTO' in df.columns:
            df[['DATA INÍCIO TREINAMENTO', 'DATA FIM TREINAMENTO']] = df['PERÍODO PREVISTO DE TREINAMENTO'].apply(parse_training_period).apply(pd.Series)

        date_cols = ['DATA DO D.O.', 'DATA DA INSTALAÇÃO', 'DATA DO INÍCIO ATEND.']
        df = parse_date_columns(df, date_cols)

        st.dataframe(df)

    except Exception as e:
        st.error(f"Erro ao processar a aba Ag. Instalação: {str(e)}")
        st.write("Verifique se a aba 'Ag_Instalacao' existe no arquivo Excel e contém as colunas esperadas.")
        st.stop()